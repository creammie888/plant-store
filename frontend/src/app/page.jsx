...
export default function Home() {
  ...
  const nextPlants = () => {
    setCurrentIndex((prevIndex) =>
      prevIndex + 4 >= plants.length ? 0 : prevIndex + 4
    );
  };

  const prevPlants = () => {
    setCurrentIndex((prevIndex) =>
      prevIndex - 4 < 0 ? plants.length - 4 : prevIndex - 4
    );
  };
          ...
          <div className="recommend-container">
            <div className="swiper-container">
              <Swiper
                      ...
                      grabCursor={true}
                    >
              {plants.slice(currentIndex).map((plant) => (
                <SwiperSlide key={plant.id}>
                <Link href={`/product/${plant.id}`} key={plant.id} className="card-reccommend">
